...
def _get_evening_window(shift: ShiftSchema) -> tuple[datetime, datetime]:
    ...
    return (datetime.combine(day, EVENING_START), datetime.combine(day, EVENING_END))
...
def calculate_pay(shifts: Iterable[ShiftSchema]) -> Dict[str, Any]:
    ...
    for shift in shifts:
        ...
        break_hours = (
            _duration_hours(
                shift.break_window.break_start, shift.break_window.break_end
            )
        ...
        overtime_hours = (
            _duration_hours(shift.shift_end, shift.overtime_end)
            if shift.overtime_end
            else 0.0
        )
        ...
        evening_hours = (
            _overlap_hours(
                shift.shift_start, shift.shift_end, evening_start, evening_end
            )
            if base_multiplier == DEFAULT_MULTIPLIER
            else 0.0
        )
